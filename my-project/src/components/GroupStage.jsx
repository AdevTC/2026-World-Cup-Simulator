...
import { Eye, Rewind, Calculator, ChevronRight, ChevronUp, ChevronDown, Layers, Eraser } from 'lucide-react';
import { Flag } from './ui/Flag';
import { TeamName } from './ui/TeamName';
import { getBestThirdsList } from '../utils/logic';

export const GroupStage = ({ groups, matches, setMatches, onNext, onClear, darkMode, setGroups }) => {
  ...
  return (
    <div className="container mx-auto p-4 animate-fade-in">
       <div className={`p-6 rounded-2xl shadow-sm border mb-8 flex flex-col lg:flex-row justify-between items-center gap-6 ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
         <div className="text-center lg:text-left">
            <h2 className="text-2xl md:text-3xl font-black flex flex-col md:flex-row items-center gap-3 tracking-tight">
         ...
         <div className="flex flex-col sm:flex-row flex-wrap justify-center items-center gap-3 w-full lg:w-auto">
            <label className={`flex items-center gap-2 cursor-pointer select-none px-4 py-2.5 rounded-xl font-bold transition shadow-sm border w-full sm:w-auto justify-center ${darkMode ? 'bg-slate-800 border-slate-700 hover:bg-slate-700' : 'bg-white border-slate-200 hover:bg-slate-50 text-slate-700'}`}>
            ...
            <div className={`hidden sm:block h-8 w-[1px] mx-2 ${darkMode ? 'bg-slate-700' : 'bg-slate-300'}`}></div>
             
             <div className="flex gap-2 w-full sm:w-auto">
                 {/* <--- NUEVO BOTÓN LIMPIAR */}
                 <button onClick={onClear} className={`flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold shadow-sm transition hover:scale-105 active:scale-95 border ${darkMode ? 'bg-slate-800 border-red-500/50 text-red-400 hover:bg-red-500/10' : 'bg-white border-red-200 text-red-500 hover:bg-red-50'}`} title="Borrar Resultados">
                   <Eraser size={18} />
                 </button>

                 <button onClick={simulateAll} className="flex-1 flex items-center justify-center gap-2 bg-red-500 hover:bg-red-600 text-white px-4 py-2.5 rounded-xl text-sm font-bold shadow-md transition hover:scale-105 active:scale-95">
                   <Rewind size={18} fill="currentColor" /> <span className="sm:hidden md:inline">Simular</span> Todo
                 </button>
                 <button onClick={simulateRest} className="flex-1 flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2.5 rounded-xl text-sm font-bold shadow-md transition hover:scale-105 active:scale-95">
                   <Calculator size={18} /> Restantes
                 </button>
             </div>
             
             <button onClick={onNext} className="w-full sm:w-auto flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-2.5 rounded-xl font-bold shadow-lg transition hover:scale-105 active:scale-95 ml-0 sm:ml-2">
               Siguiente <ChevronRight size={20} />
             </button>
         </div>
       </div>
       
       {/* ... (El resto de la tabla de grupos se mantiene idéntico) ... */}
                  ...
                  <table className="w-full text-xs min-w-[300px]">
                    ...
                    <tbody className={`divide-y ${darkMode ? 'divide-slate-800' : 'divide-slate-50'}`}>
                      {groups[gLetter].map((t, idx) => {
                        ...
                        if (idx < 2) { barColor = 'bg-emerald-500'; rowClass = darkMode ? 'bg-emerald-500/10' : 'bg-emerald-50'; } 
                        else if (idx === 2) { const isThird = qualifiedThirds.some(q => q.name === t.name); if (isThird) { barColor = 'bg-emerald-500'; rowClass = darkMode ? 'bg-emerald-500/5' : 'bg-emerald-50/40'; } }
                        return (
                            ...
                            <td className="p-2 pl-1 flex items-center relative">
                                ...
                                <div className="pl-3 flex items-center">
                                    ...
                                    <div className="ml-2"><TeamName name={t.name} pot={t.pot} showPot={showPots} /></div>
                                </div>
                            </td>
                ...
                {matches[gLetter].map(m => (
                  <div key={m.id} className={`flex items-center justify-between text-xs p-2 rounded-lg transition border ${darkMode ? 'hover:bg-white/5 border-transparent' : 'bg-white border-slate-200 hover:border-blue-200 shadow-sm'}`}>
                    ...
                    <div className="flex gap-2 items-center justify-center w-1/3">
                      <input type="number" placeholder="-" className={`w-9 h-8 text-base text-center rounded-md font-mono font-bold border-2 outline-none transition-all focus:scale-110 focus:z-10 ${darkMode ? 'bg-slate-800 border-slate-700 text-white focus:border-blue-500' : 'bg-slate-50 border-slate-200 text-slate-900 focus:border-blue-500 focus:bg-white'}`} value={m.scoreA} onKeyDown={preventInvalidInput} onChange={(e) => handleScore(gLetter, m.id, 'scoreA', e.target.value)} />
                      <input type="number" placeholder="-" className={`w-9 h-8 text-base text-center rounded-md font-mono font-bold border-2 outline-none transition-all focus:scale-110 focus:z-10 ${darkMode ? 'bg-slate-800 border-slate-700 text-white focus:border-blue-500' : 'bg-slate-50 border-slate-200 text-slate-900 focus:border-blue-500 focus:bg-white'}`} value={m.scoreB} onKeyDown={preventInvalidInput} onChange={(e) => handleScore(gLetter, m.id, 'scoreB', e.target.value)} />
                    </div>
