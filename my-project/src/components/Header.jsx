...
import { Trophy, RefreshCw, Sun, Moon } from 'lucide-react';
import { TROPHY_URL } from '../data/constants';

export const Header = ({ darkMode, setDarkMode, reset, view }) => (
  <header className={`p-4 flex justify-between items-center shadow-md print:hidden ${darkMode ? 'bg-slate-900 text-white' : 'bg-white text-slate-900'}`}>
    ...
    <div className="flex gap-4">
      {view !== 'landing' && (
        <button onClick={reset} className="flex items-center gap-2 text-sm font-semibold hover:text-red-500 transition">
